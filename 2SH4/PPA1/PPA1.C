...
char dispString[STRING_SIZE] = {0}; // The display String
int position; // print position
int direction; // direction of the string (left to right, right to left)
int startIndex; //startIndex is input index
...
void Initialize(void)
{
    ...
    startIndex = 2; // user will input in the 2nd index 
    direction = 0; //intialize the print direction from right to left
    position = 0; // user will 
...
void RunLogic(void)
{
    ...
    //if spacebar is pressed then change direction
    else if (cmd == 32){
        if (direction == 0) {
            direction = 1;
        } else {
    ...
    //if alphanumerical value is entered then display on screen
    else if(((cmd > 47 && cmd < 58) || (cmd > 64 && cmd < 91) || (cmd > 96 &&  cmd < 123)) && (startIndex < STRING_SIZE - 2)) {
        dispString[startIndex] = cmd;
        dispString[startIndex + 2] = '<';
        startIndex++;
   }
   cmd = 0;
...
void DrawScreen(void)
{
    ...
    for(int i = 0; i < STRING_SIZE; i++) {
        MacUILib_printf("%c", dispString[(position+i) % STRING_SIZE]);
    }
    MacUILib_printf("\n====================");
